# coding: utf-8

"""
    Lob

    The Lob API is organized around REST. Our API is designed to have predictable, resource-oriented URLs and uses HTTP response codes to indicate any API errors. <p> 

    The version of the OpenAPI document: 1.19.28
    Contact: lob-openapi@lob.com
    Created by: https://support.lob.com/
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from lob_python_sdk.type.city import City
from lob_python_sdk.type.postal_code import PostalCode
from lob_python_sdk.type.state import State

class RequiredIntlComponents(TypedDict):
    pass

class OptionalIntlComponents(TypedDict, total=False):
    # The numeric or alphanumeric part of an address preceding the street name. Often the house, building, or PO Box number.
    primary_number: str

    # The name of the street.
    street_name: str

    city: City

    state: State

    postal_code: PostalCode

class IntlComponents(RequiredIntlComponents, OptionalIntlComponents):
    pass
